...
`fiona.db` 与 `uploads/` 必须作为一个隐私数据集合共同备份和清理。“清空历史”只删消息；设置页的“删除账户”才会在事务中删除全部账号关联记录。无引用上传文件随后删除，失败项保存在 `upload_cleanup_queue`，后端启动时重试。新帖子在库内保存 owner；能通过当前 HMAC 或旧 MD5 匿名标识识别的历史帖子会自动回填，无法识别的旧行仍需人工处置。
...
候选会经过模型评估，再写入待接受卡片和匹配记录。双方都接受后才能进入真人房间；Peer WebSocket 会再次校验房间成员资格。
...
- 账户删除已有引用检查和持久化文件清理重试；其他孤儿文件、保留期限和周期性清理尚未完成。
- ASGI 层限制总请求体为 25 MB，Chat/图片、ASR、TTS、帖子和分页还有更严格的领域边界；模型并发、公共热点缓存和持久化成本上限仍需继续治理。
- 模型生成的热点/卡片 URL 通过只允许公网 HTTP(S) 的客户端访问，每次 DNS 和重定向都会重新校验并限制响应体与超时。
- 前端主页面承担职责过多，多个隐藏 iframe 会继续运行轮询或 3D 场景。
- 当前 CI 只构建 Windows 桌面端，尚未覆盖后端和 Web 质量门禁。
